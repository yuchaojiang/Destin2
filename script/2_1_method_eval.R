...
for(data in c('bmmc','pbmc','brain','human')){
  ...
  p1=DimPlot(object = obj, group.by='consensus_clusters', label = TRUE, reduction = 'consensus.umap') + NoLegend()+ggtitle('Consensus PCA')
  p2=DimPlot(object = obj, group.by='predicted.id', label = TRUE, reduction = 'consensus.umap') + NoLegend()+ggtitle('Consensus PCA')
  p1+p2
  ...
  print(output)
  cat('\n\n\n')
  write.csv(output, file=paste0('metrics/metrics_output_',data,'.csv'), row.names = T)
  
  save(obj, file=paste0(data,'_processed_analyzed.rda'))
...
# Except for cLISI, the higher the better (hence change them to negative)
results.all.neg=results.all
results.all.neg[,!grepl('cLISI', colnames(results.all))]=-results.all[,!grepl('cLISI', colnames(results.all))] 
results.all.rank=apply(results.all.neg,2,rank)

pdf(file='pdf/metrics.pdf', width=12, height=3)
par(mar=c(7.1, 4.1, 4.1, 2.1))
par(mfrow=c(1,5))
boxplot(t(results.all.rank), las=2,
        col=c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00"),
        main='All metrics', ylab='Rank (smaller = better)')
for(i in 1:4){
